  ...
  try {
    const response = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: 'Welcome to WorkNest - Your Temporary Credentials',
        htmlContent: `
          <div style="font-family: sans-serif; padding: 20px; line-height: 1.6; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
            <h2 style="color: #2563eb; margin-bottom: 20px; font-weight: 800; font-size: 24px;">Welcome to WorkNest, ${name}!</h2>
            <p style="color: #334155; font-size: 15px;">Your WorkNest account has been successfully created. Below are your temporary login credentials:</p>
            ...
            <div style="text-align: center; margin: 32px 0;">
              <a href="${process.env.CLIENT_ORIGIN || 'http://localhost:5173'}/login" style="background-color: #2563eb; color: #ffffff !important; padding: 12px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; box-shadow: 0 4px 6px -1px rgba(37, 99, 235, 0.2);">Log In to WorkNest</a>
            </div>
